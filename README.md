...
Every production build is first written to `.tmp-build/<mode>/<buildId>/dist`, validated, and only then promoted to `dist`. Promotion never renames, moves, or deletes the canonical `dist` directory: it copies and verifies the new non-entry files and manifest, then atomically replaces `index.html` last. `sharepoint-deploy-manifest.json` records the `buildId`, file size, SHA-256, and the current local references in `index.html`. If Vite, a file copy, or the final index replacement fails, the previous entry point remains active and the failed staging path plus exact locked file are logged. Stale hashed assets are cleaned only after success and cleanup locks are warnings, never promotion failures.

The legacy deploy command validates that manifest. It copies and verifies every non-entry file first, then commits `index.html` last. Bootstrap deployment mirrors only the configured bootstrap `dist` directory; final deployment preserves old hashed chunks until the current build is completely verified.
...
Upload that resulting `dist` to Release Manager. This build contains no site-specific SharePoint identity. Release Manager supplies `sitebuilder-runtime-config.json` at deployment time, and the universal frontend fails safely with `missing_runtime_config` when that configuration is absent.
