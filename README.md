...
The universal release manifest is generated by `build:universal` only after isolated staging validation and includes a unique `buildId`, per-file SHA-256, and `requiresDataSchemaVersion` for backward-compatible contract checks. `npm run verify:universal-dist` proves that target-specific runtime overlays do not change its JS/CSS bytes.
...
- `GET /healthz` ו-`GET /api/healthz` נשארים public-compatible ומחזירים גם `storageBackend`, `appVersion`, `gitCommit`, `dataSchemaVersion`, `supportedFrontendRange`.
- `GET /api/sites/:siteId/provision-status` בודק אם רישום האתר וקבצי ברירת המחדל הקנוניים כבר קיימים, בלי לחשוף תוכן, hash-ים של תוכן או סודות.
- `POST /api/sites/:siteId/provision` הוא endpoint מאומת, idempotent ולא-הרסני: הוא יוצר רק את רישום האתר ו-default legacy objects שחסרים, ולעולם לא דורס נתונים קיימים.
