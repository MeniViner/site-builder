...
The legacy build compiles `VITE_SP_*`, `VITE_SITE_BASE_URL`, storage selection, and site identity into its frontend. It creates the frozen SharePoint descriptor from those values at startup and does **not** fetch or require `sitebuilder-runtime-config.json`, `runtime-config.json`, or another deployment overlay. The deploy command simply copies that site-specific `dist` to the configured SharePoint target.
...
The universal release manifest is generated by `build:universal`; `npm run verify:universal-dist` proves that target-specific runtime overlays do not change its JS/CSS bytes.
