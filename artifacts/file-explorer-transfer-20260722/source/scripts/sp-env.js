...
export function resolveConfig({ envFilePath = path.resolve(process.cwd(), '.env.production'), cli = {}, environment = process.env } = {}) {
  ...
  const fileExplorerApiUrl = pick(
    cli['file-explorer-api-url'] || cli.fileExplorerApiUrl,
    environment.VITE_FILE_EXPLORER_API_URL || envFromFile.VITE_FILE_EXPLORER_API_URL,
    '',
  ).replace(/\/+$/g, '');
...
export function writeEnvProduction(config, outputPath = path.resolve(process.cwd(), '.env.production')) {
  const lines = [
    ...
    `VITE_FILE_EXPLORER_API_URL=${config.fileExplorerApiUrl || ''}`,
