...
const MESSAGES = Object.freeze({ access_denied: 'לחשבון השירות אין הרשאה לקרוא נתיב זה.', blocked_root: 'הנתיב המבוקש אינו נמצא באחד השורשים המורשים.', explorer_disabled: 'סייר הקבצים אינו מוגדר או אינו זמין בשרת זה.', file_too_large: 'הקובץ גדול מהמגבלה המותרת להצגה או להורדה.', invalid_path: 'הנתיב המבוקש אינו תקין.', not_found: 'הקובץ או התיקייה לא נמצאו.', timeout: 'הפעולה ארכה זמן רב מדי. נסו שוב.', unavailable_share: 'שיתוף הקבצים אינו זמין כרגע.', unsupported_operation: 'פעולה זו אינה זמינה בסייר הארגוני.' });
...
export function createFileExplorerRouter({ config = {} } = {}) {
  ...
  const router = Router();
  router.get('/readiness', (_req, res) => { const readiness = getFileExplorerReadiness({ ...config, ...explorer }); return res.status(readiness.ready ? 200 : 503).json({ ok: readiness.ready, readiness }); });
